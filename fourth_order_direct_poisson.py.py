...
def filter_density(rho):
    ...
    num = np.tanh(beta_proj * eta_proj) + ufl.tanh(beta_proj * (rhof - eta_proj))
    return num / den
...
def solve_case(rho_phys, eps_xx, eps_yy, gamma_xy, name):
    ...
    F = I + grad(w + ubar)
    theta = rho_min + (1.0 - rho_min) * rho_phys**penal
    mu = mu0 * theta
    lmbda = lmbda0 * theta

    E = 1./2.*( F.T*F - I )
    C = as_tensor( lmbda*I[i,j]*I[k,l] + mu*( I[i,k]*I[j,l] + I[i,l]*I[j,k]) , [i,j,k,l])

    P = as_tensor(C[i,j,k,l]*E[k,l] , [i,j])
...
def eval_cb(_j, m_rho):
    rp = filter_density(m_rho)
    assign(rho_viz, project(rp, Q))
    rho_file << rho_viz


rho = initialize_density()
rho_phys = filter_density(rho)

output_dir = "output 1.5.5"
